...
public:
    ...
    void init_graph()
    {
        ...
        std::string serialize_graph = data_folder + FILESEP + "serilized.graph";
        std::string serialize_rgraph = data_folder + FILESEP + "serilized.rgraph";


        if (seraizlied_graph_exist(serialize_graph) && seraizlied_graph_exist(serialize_rgraph))
        {

            load_serialized_graph(serialize_graph, g);
            load_serialized_graph(serialize_rgraph, gr);
        ...
        if (!seraizlied_graph_exist(serialize_graph))
        {
            std::cout << "file name: " << serialize_graph << std::endl;
            save_serialized_graph(serialize_graph, g);
        }

        if (!seraizlied_graph_exist(serialize_rgraph))
        {
            std::cout << "file name: " << serialize_rgraph << std::endl;
            save_serialized_graph(serialize_rgraph, gr);
        }
    }
