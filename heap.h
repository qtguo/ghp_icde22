...
//        cout << comp(2, 1) << endl;
    ...
    void insert(int index, T element) {
        ...
        heap.push_back(make_pair(element, index));
    ...
    void delete_a_node(int index){
        ...
        index = heap_idx[index];

        heapswap(index, (int) (heap.size() - 1));
        heap_idx[heap.back().second] = -1;
        heap.pop_back();

        //if the new node is larger than the value of its parent node
        //then heapifydown.
        //else heapifyup/
        ...
        if(parent_idx == -1){
            ...
            if(comp(heap[index].first, heap[parent_idx].first)){
                heapifydown(index);
            }
            else{
                heapifyup(index);
